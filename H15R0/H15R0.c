...
const CLI_Command_Definition_t percentagevalueCommandDefinition ={(const int8_t*) "percentage" , /* The command string to type. */
...
const CLI_Command_Definition_t AnalogvalueCommandDefinition ={(const int8_t*) "analogval" , /* The command string to type. */
...
#ifdef H15R0
	if(huart->Instance == USART5) return P1;
	else if(huart->Instance == USART2) return P2;
	else if(huart->Instance == USART3) return P3;
	else if(huart->Instance == USART1) return P4;
#endif
...
portBASE_TYPE percentagevalueCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString){
	...
	static const int8_t *pcOKMessage =(int8_t*) "OP-AM  is at percentage value  %0.2f%%\r\n";
	...
	outputVoltage =(float) atof((char*) pcParameterString1);
	
	result =AnalogPercentage(outputVoltage);
	
	/* Respond to the command */
	if(result == H15R0_OK) sprintf((char*) pcWriteBuffer,(char*) pcOKMessage,outputVoltage);
	else if(result == H15R0_ERR_WrongParams) strcpy((char*) pcWriteBuffer,(char*) pcWrongoutputVoltageMessage);
...
portBASE_TYPE AnalogvalueCommand(int8_t *pcWriteBuffer, size_t xWriteBufferLen, const int8_t *pcCommandString){
	...
	portBASE_TYPE xParameterStringLength1 =0;
	float outputVoltage =0;
	static const int8_t *pcOKMessage =(int8_t*) "OP-AM  is at precise value %0.2f\r\n";
	static const int8_t *pcWrongoutputVoltageMessage =(int8_t*) "Wrong outputVoltage!\n\r";
